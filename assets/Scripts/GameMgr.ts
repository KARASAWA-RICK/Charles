import { _decorator, Component, Node, EventTarget, Prefab, instantiate, find, Vec3, view, Vec2, v2, Collider2D, IPhysics2DContact, PhysicsSystem2D, Contact2DType, math, v3, director, Canvas } from 'cc'
import { Tools } from './Tools'
import { BOSS_TYPE, BULLET_TYPE, COLLIDER2D_TYPE, Events, Global, ITEM_TYPE, ui } from './Global';
...
export class GameMgr extends Component {
    ...
    CreateEnemy() {
        ...
        while (attempts < this.maxAttempts) {
            ...
            //计算敌人与玩家的距离
            distance = Tools.calculateDistance(Tools.convertTo2D(pos), Tools.convertTo2D(this.hero.position))
    ...
    CreateBoss() {
        ...
        while (attempts < this.maxAttempts) {
            ...
            if (distance > this.radius) {
                ...
                //根据Boss种类与朝向初始化Boss
                //Boss初始角度朝向玩家
                let angle = Tools.convertToAngle(Tools.calculateDir(Tools.convertTo2D(boss.position), Tools.convertTo2D(this.hero.position)))
    ...
    clearViews() {
        this.node.getChildByName('Layer2').getComponentInChildren(GameView)?.close()
        this.node.getChildByName('Layer3').getComponentInChildren(HomeView)?.close()
        this.node.getChildByName('Layer4').getComponentInChildren(RankView)?.close()
        this.node.getChildByName('Layer4').getComponentInChildren(ResultView)?.close()
        this.node.getChildByName('Layer4').getComponentInChildren(PauseView)?.close()
    }
    ...
    clear(dotsClearType: number) {
        ...
        switch (dotsClearType) {
            case 1: this.clearDots()
                break
            case 2: this.clearDotsOnDead()
                break
            default: this.clearDots()
                break
        }
        this.clearItemKills()
        //清理界面
    ...
    start() {
        ...
        //打开事件监听
        console.log('打开事件监听')
        Global.eventTarget.on(Events.StartPlay, () => { this.startPlay() })
        Global.eventTarget.on(Events.Die, () => { this.afterDeath() })
    ...
    onDisable() {
        this.unscheduleAllCallbacks()
        Global.eventTarget.off(Events.StartPlay, () => { this.startPlay() })
        Global.eventTarget.off(Events.Die, () => { this.afterDeath() })
