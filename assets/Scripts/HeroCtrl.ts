import { _decorator, Component, Node, Vec2, Vec3, view, Size, Collider2D, Contact2DType, IPhysics2DContact, instantiate, Prefab, Sprite, BoxCollider2D, SpriteFrame, UITransform, size, PhysicsSystem2D, v3, tween, quat, Quat, find, input, EventTouch, Input, v2, lerp, math, director, game } from 'cc'
import { Tools } from './Tools'
import { Global, ITEM_TYPE, COLLIDER2D_TYPE, BULLET_TYPE, Events } from './Global'
...
export class HeroCtrl extends Component {
    ...
    onBeginContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        //玩家碰Dot
        if ((selfCollider.tag == COLLIDER2D_TYPE.Dot && otherCollider.tag == COLLIDER2D_TYPE.Hero) || (selfCollider.tag == COLLIDER2D_TYPE.Hero && otherCollider.tag == COLLIDER2D_TYPE.Dot)) {
            //玩家立即死亡
            this.Die()
        }

        //玩家碰道具
        else if ((selfCollider.tag == COLLIDER2D_TYPE.Item && otherCollider.tag == COLLIDER2D_TYPE.Hero) || (selfCollider.tag == COLLIDER2D_TYPE.Hero && otherCollider.tag == COLLIDER2D_TYPE.Item)) {
            //改变拾取道具状态，道具销毁
        ...
        //Dot碰子弹
        else if ((selfCollider.tag == COLLIDER2D_TYPE.Bullet && otherCollider.tag == COLLIDER2D_TYPE.Dot) || (selfCollider.tag == COLLIDER2D_TYPE.Dot && otherCollider.tag == COLLIDER2D_TYPE.Bullet)) {
            //敌人死亡
            //如果子弹是大炮子弹，子弹销毁
            if (selfCollider.tag == COLLIDER2D_TYPE.Dot) {
                GameMgr.ins.DotDie(selfCollider.node)
                GameMgr.ins.coin = GameMgr.ins.coin + 1
                if (otherCollider.node.getComponent(BulletCtrl).bulletType == BULLET_TYPE.Gun) {
                    PoolMgr.ins.putNode(otherCollider.node)
                }
            } else {
                GameMgr.ins.DotDie(otherCollider.node)
                GameMgr.ins.coin = GameMgr.ins.coin + 1
                if (selfCollider.node.getComponent(BulletCtrl).bulletType == BULLET_TYPE.Gun) {
                    PoolMgr.ins.putNode(selfCollider.node)
                }
            }
        }
        ...
        //Dot碰杀伤性道具
        else if ((selfCollider.tag == COLLIDER2D_TYPE.Dot && otherCollider.tag == COLLIDER2D_TYPE.KillItem) || (selfCollider.tag == COLLIDER2D_TYPE.KillItem && otherCollider.tag == COLLIDER2D_TYPE.Dot)) {
            //敌人死亡
            //如果道具是盾，盾失效，生成盾爆
    ...
    bigInit() {
        ...
        for (let i = 0; i < angleArr.length; i++) {
            //Bullet出池
            let bullet: Node = PoolMgr.ins.getNode('Bullet', this.bullets, this.node.position)
            //设置子弹角度
    ...
    shieldBombInit() {
        //ShieldBomb出池
        let bomb = PoolMgr.ins.getNode('ShieldBomb', this.itemKills, this.node.position)
        //渐入效果，0.2秒内恢复原本尺寸
        Tools.fadeIn(bomb, 0.2)
        //console.log('盾爆生成')
        this.scheduleOnce(() => {
    ...
    pushScreenInit() {
        //实例化推推
        let pushScreen = PoolMgr.ins.getNode('PushScreen', this.itemKills, v3(-GameMgr.ins.WIDTH / 2, 0, 0))
        //console.log('推推生成')
        tween(pushScreen).by(3, { position: v3(3000, 0, 0) }).call(() => { PoolMgr.ins.putNode(pushScreen) }).start()
    }

    //生成吃豆人
    packManInit() {
        //实例化吃豆人
        let packMan = PoolMgr.ins.getNode('Packman', this.itemKills, this.node.position)
    ...
    followFireInit() {
        //实例化跟踪火
        let followFire = PoolMgr.ins.getNode('FollowFire', this.itemKills, this.node.position)
        //5S后熄灭
        this.scheduleOnce(() => {
    ...
    onTouchMove(event: EventTouch) {
        ...
        if (!this.node.getChildByName('Gun').active) {
            //获取玩家新位置
            let newPos = new Vec3(this.node.position.x + this.dirMove.x, this.node.position.y + this.dirMove.y, 0)
            //防止超出边界
            newPos.x = Math.min(Math.max(newPos.x, (-this.borderSize.x + 100) / 2), (this.borderSize.x - 100) / 2)
            newPos.y = Math.min(Math.max(newPos.y, (-this.borderSize.y + 400) / 2), (this.borderSize.y - 400) / 2)
            //更新玩家位置
    ...
    isStartPlay() {
        if (GameMgr.ins.isStartPlay) {
            //console.log('已经开始辣')
            return
        } else if ((Tools.calculateDistance(Tools.convertTo2D(this.node.position), Tools.convertTo2D(this.posStart)) >= this.startDistance)) {
